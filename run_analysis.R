...
ind <- ind + 2    # pushing column indx up 2 for the subject/activity cols(1&2)
ind2 <- grep("std", df_featurevar[,2], value=FALSE)
ind2 <- ind2 + 2  # pushing col indx up 2 for subj / activity
## need to add cols 1 & 2 (subject & activity) to vector
indcomb <- as.vector(rbind(c(1:2, ind))) ##now add ind2
indcomb2 <- as.vector(rbind(indcomb, ind2))
## now sort
indcombined <- sort(unique(indcomb2))
## indcombined is now a vector of column indices containing mean or std (in order) including cols 1&2

## extract mean/std columns using indices in indcomb2
Extractdf <- df_combined[,indcombined]
